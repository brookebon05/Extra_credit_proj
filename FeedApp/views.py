...
def profile(request):
    ...
    if request.method != "POST":  # this means its get
...
def myfeed(request):
    ...
    zipped_list = zip(posts, comment_count_list, like_count_list)  # getting all stuff
...
def friendsfeed(request):
    ...
    if request.method == "POST" and request.POST.get("like"):
        post_to_like = request.POST.get("like")  # gets p-id
        print(post_to_like)
...
def friends(request):
    ...
    # to get friend requests received by user
    ...
    if request.method == "POST" and request.POST.get("receiver_requests"):
        senders = request.POST.getlist("friend_requests")
        for sender in senders:
            Relationship.objects.filter(id=sender).update(status="accepted")
            relationsip_obj = Relationship.objects.get(id=sender)
            user_profile.friends.add(relationsip_obj.sender.user)

            # add user to list
            relationsip_obj.sender.friends.add(request.user)
